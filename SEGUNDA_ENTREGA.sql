...
--Se suma el costo del pasaje al costo total de la compra asociada al mismo
 ...
 BEGIN
	  ...
	  --Calculo el precio de la habitacion, que supone un 20% de ganancia, por lo que es habitacion_costo + 0.20*habitacion_costo
	  SELECT @precio_hab = (he.HABITACION_COSTO + he.HABITACION_COSTO*0.20)
	  FROM Habitacion_X_Estadia he, inserted i
	  WHERE he.ESTADIA_CODIGO = i.ESTADIA_CODIGO
	  AND he.ID_HABITACION = i.ID_HABITACION;
	  ...
	  SELECT i.ID_FACT_ESTADIA,i.FACTURA_NRO,i.ESTADIA_CODIGO,i.ID_HABITACION,i.FECHA_CHECK_IN,i.FECHA_CHECK_OUT, @precio_hab FROM inserted i;
 END;
 GO

 --Calculo el precio del pasaje antes de instanciarlo. El mismo es el costo del pasaje + el 20% del mismo
 CREATE TRIGGER GESTIONADOS_POR_LA_PANDEMIA.calcular_precio_pasaje
 ON GESTIONADOS_POR_LA_PANDEMIA.Pasaje_Facturado INSTEAD OF INSERT
 AS
 DECLARE @precio_pas DECIMAL(18,2) = 0;
 BEGIN
      SELECT @precio_pas = (p.PASAJE_COSTO + p.PASAJE_COSTO*0.20)
	  FROM Pasaje p, inserted i
	  WHERE i.PASAJE_CODIGO = p.PASAJE_CODIGO;

	  INSERT INTO GESTIONADOS_POR_LA_PANDEMIA.Pasaje_Facturado(ID_FACT_PASAJE,FACTURA_NRO,PASAJE_CODIGO,PASAJE_PRECIO)
	  SELECT i.ID_FACT_PASAJE, i.FACTURA_NRO, i.PASAJE_CODIGO, @precio_pas FROM inserted i;
 END;
 GO
 ...
 ON GESTIONADOS_POR_LA_PANDEMIA.Estadia_Facturada(ID_HABITACION);
 ...
 ON GESTIONADOS_POR_LA_PANDEMIA.Habitacion(HOTEL_RAZON_SOCIAL);
